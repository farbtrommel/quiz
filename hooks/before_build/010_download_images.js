...
function resizeImage(src, dest, mindest, crop) {
  ...
  easyimg.exec(cmd).then(function () {
    ...
    deferred.reject({err: err, src: src, dest: dest, mindest: mindest, crop: crop});
  });
...
function readFileToObj(path) {
  var deferred = Q.defer();
  //var parseArrayExp = /.\/\/*@data.*return\s*(\[.*\])\s*;\/\/@enddata.*/;
  //Read Data
...
if (rootdir) {
  //Read Quiz Data
  /*
  readFileToObj(rootdir + '/app/quiz/controller.ts').then(function (obj) {
  ...
  */
  var filePath = rootdir + '/app/quiz/data.json';
