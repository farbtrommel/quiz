...
export class QuizSetGenerate {
  ...
  private gameSetById:{[id:number]: IGameEntry} = {};
  private gameSetByCat:{[id:number]: IGameEntry[]} = {};
  /**
   * Helper
   * @type {Array}
   */
  private excludeGameEntries:number[] = [];
  /**
  ...
  constructor(quizSet:QuizSet, gameSet:IGameEntry[], storageService:StorageService, callback:() => void) {
    ...
    //no pointer, store a copy
    this.gameSet = JSON.parse(JSON.stringify(gameSet));
    ...
    if (this.expertMode) {
      ...
      for (var cat in catFromItem) {
        if (this.gameSetByCat[cat] && this.gameSetByCat[cat].length > 0) {
          return this.pickElementAndRemoveFromOrigin(parseInt(cat));
        }
      }
    }
    ...
    for (var key in keys) {
      ...
      if (!skip) {
        if (this.gameSetByCat[key] && this.gameSetByCat[key].length > 0) {
          return this.pickElementAndRemoveFromOrigin(parseInt(key));
        }
      }
    }
  ...
  createGameEntryListByCategory() {
    if (this.gameSet && this.gameSet.length == 0) {
      this.gameSet = Quiz.getGameSetById(this.quizSet.GameId).GamesSet;

      //remove all used entries from list.
      for (var i = 0; i < this.gameSet.length; i++) {
        for (var id in this.excludeGameEntries) {
          if (id == this.gameSet[i].id.toString()) {
            this.gameSet.splice(i, 1);
            break;
          }
        }
    ...
    for (var i:number = 0; i < this.gameSet.length; i++) {
      ...
      for (var cat in item.Categories) {
        QuizSetGenerate.savePush(this.gameSetByCat, parseInt(cat), item);
      }
    }
